...
#recompensa
    ...
    def get_average_acceleration(self) -> float:
        """Returns the average acceleration of vehicles in the intersection."""
        avg_acceleration = 0.0
        vehs = self._get_veh_list()
        if len(vehs) == 0:
            return 0.0  # Si no hay vehículos en la intersección, la aceleración promedio es cero
        for v in vehs:
            # La aceleración de un vehículo es el cambio de velocidad dividido por el intervalo de tiempo
            acceleration = self.sumo.vehicle.getAcceleration(v)
            avg_acceleration += acceleration
        return avg_acceleration / len(vehs)

    def get_accumulated_VSP_emissions(self) -> float:
        """Returns the accumulated VSP emissions for vehicles passing through the intersection."""
        accumulated_VSP_emissions = 0.0
        vehs = self._get_veh_list()
        for v in vehs:
            speed = self.sumo.vehicle.getSpeed(v)
            acceleration = self.sumo.vehicle.getAcceleration(v)
            VSP_emission = speed * (1.1 * acceleration + 0.132) + 0.000302 * speed ** 3
            accumulated_VSP_emissions += VSP_emission
        return accumulated_VSP_emissions
    ...
    # def get_co2(self):
    #     """Returns the total CO2 emissions (#veh leaving - #veh approaching) of the intersection."""
    #     CO2_outgoing = sum(self.sumo.vehicle.getCO2Emission(vehID) for lane in self.out_lanes for vehID in self.sumo.lane.getLastStepVehicleIDs(lane))
    #     CO2_approaching = sum(self.sumo.vehicle.getCO2Emission(vehID) for lane in self.lanes for vehID in self.sumo.lane.getLastStepVehicleIDs(lane))
    #     return CO2_outgoing - CO2_approaching

    def get_co2(self):
        """Returns the total CO2 emissions for the entire simulation."""
        total_co2 = sum(self.sumo.vehicle.getCO2Emission(vehID) for vehID in self.sumo.vehicle.getIDList())
        return total_co2
