...
class SumoEnvironment(gym.Env):
    ...
    def __init__(
        ...
        # if LIBSUMO:
        #     traci.start([sumolib.checkBinary("sumo"),"-c", self._sumocfg])  # Start only to retrieve traffic light information
        #     conn = traci
        # else:
        #     traci.start([sumolib.checkBinary("sumo"),"-c", self._sumocfg], label="init_connection" + self.label)
    ...
    def _get_system_info(self):
        ...
        speeds = np.array([self.sumo.vehicle.getSpeed(vehicle) for vehicle in vehicles])
        waiting_times = [self.sumo.vehicle.getWaitingTime(vehicle) for vehicle in vehicles]
        accelerations = np.array([self.sumo.vehicle.getAcceleration(vehicle) for vehicle in vehicles])
        VSP_emissions = speeds * (1.1 * accelerations + 0.132) + 0.000302 * (speeds ** 3) # C02
        CO2_emissions = [self.sumo.vehicle.getCO2Emission(vehicle) for vehicle in vehicles]  # C02
        fuel_consumption =  [self.sumo.vehicle.getFuelConsumption(vehicle) for vehicle in vehicles]  # C02   
        
      #  accel =  [self.sumo.vehicle.getAccel(vehicle) for vehicle in vehicles] 
      #  decel =  [self.sumo.vehicle.getDecel(vehicle) for vehicle in vehicles] 
        out_lanes_densities = []
        lanes_densities = []
        lanes_queue = []
        total_queued = 0
        for ts in self.ts_ids:
            out_lanes_densities.extend(self.traffic_signals[ts].get_out_lanes_density())
            lanes_densities.extend(self.traffic_signals[ts].get_lanes_density())
            lanes_queue.extend(self.traffic_signals[ts].get_lanes_queue())
            
        # Calcular la densidad promedio de los carriles de salida
        out_lanes_density_avg = 0.0 if len(out_lanes_densities) == 0 else np.mean(out_lanes_densities)
        lanes_density_avg = 0.0 if len(lanes_densities) == 0 else np.mean(lanes_densities)
        lanes_queue_avg = 0.0 if len(lanes_queue) == 0 else np.mean(lanes_queue)        
        total_queued=[self.traffic_signals[ts].get_accumulated_waiting_time_per_lane() for ts in self.ts_ids]
        
        return {
            # In SUMO, a vehicle is considered halting if its speed is below 0.1 m/s
            "system_vehicles": len(vehicles),
            "system_total_stopped": sum(int(speed < 0.1) for speed in speeds),
            "system_total_waiting_time": sum(waiting_times),
            "system_VSP": sum(VSP_emissions),
            "system_mean_waiting_time": 0.0 if len(vehicles) == 0 else np.mean(waiting_times),
            "system_mean_speed": 0.0 if len(vehicles) == 0 else np.mean(speeds),
            "system_mean_acceleration": 0.0 if len(vehicles) == 0 else np.mean(accelerations),
            "system_mean_VSP": 0.0 if len(vehicles) == 0 else np.mean(VSP_emissions),
            "system_total_C02_emissions": sum(CO2_emissions),  # CO2           
            "system_mean_fuel_consumption": 0.0 if len(vehicles) == 0 else np.mean(fuel_consumption),
            "system_total_fuel_consumption": sum(fuel_consumption),
       #     "system_mean_accel": 0.0 if len(vehicles) == 0 else np.mean(accel),
       #     "system_mean_decel": 0.0 if len(vehicles) == 0 else np.mean(decel),
            "system_out_lanes_density_avg ": out_lanes_density_avg, 
            "system_lanes_density_avg ": lanes_density_avg,
            "system_queue_avg ": lanes_queue_avg, 
#            "system_mean_total_queued": 0.0 if len(vehicles) == 0 else np.mean(total_queued)
    ...
    #     ]
    #     average_speed = sum(self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids) / len(self.ts_ids)
    #     average_acceleration = sum(self.traffic_signals[ts].get_average_acceleration() for ts in self.ts_ids) / len(self.ts_ids)      
    #     info = {}
    ...
    def _get_per_agent_info(self):
        ...
        average_speed = sum(self.traffic_signals[ts].get_average_speed() for ts in self.ts_ids) / len(self.ts_ids)
        average_acceleration = sum(self.traffic_signals[ts].get_average_acceleration() for ts in self.ts_ids) / len(self.ts_ids)
        #VSP_emissions = average_speed * (1.1 * average_acceleration + 0.132) + 0.000302 * average_speed ** 3
        # Calculating accumulated VSP emissions
        accumulated_VSP_emissions = sum(self.traffic_signals[ts].get_accumulated_VSP_emissions() for ts in self.ts_ids)
       # get_out_lanes_density = sum(self.traffic_signals[ts].get_out_lanes_density() for ts in self.ts_ids)
        info = {}
        for ts in self.ts_ids:
            info[f"{ts}_stopped"] = stopped[self.ts_ids.index(ts)]
            info[f"{ts}_accumulated_waiting_time"] = accumulated_waiting_time[self.ts_ids.index(ts)]
            info[f"{ts}_average_speed"] = average_speed
            info[f"{ts}_average_acceleration"] = average_acceleration
            
        info["agents_total_stopped"] = sum(stopped)
        info["agents_total_accumulated_waiting_time"] = sum(accumulated_waiting_time)
        info["total_accumulated_VSP_emissions"] = accumulated_VSP_emissions
       # info["system_total_lanes_density"]= sum(get_out_lanes_density)
